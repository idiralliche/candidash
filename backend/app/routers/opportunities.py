...
def get_opportunities(
    ...
    """
    Retrieve a list of opportunities with pagination and optional filtering.
...
def create_opportunity(opportunity: OpportunityCreate, db: Session = Depends(get_db)):
    ...
    - **position_type**: Role type (required)
    ...
    if opportunity.company_id is not None:
        ...
        company = db.query(CompanyModel).filter(CompanyModel.id == opportunity.company_id).first()
        if not company:
            raise HTTPException(status_code=404, detail=f"Company with id {opportunity.company_id} not found")

    db_opportunity = OpportunityModel(**opportunity.model_dump())
